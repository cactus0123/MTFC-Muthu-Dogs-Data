...
def get_coordinates(index, row):
    # Skip rows when errors occur
    if index < 218:
        return row

    if row["City"] == "San Diego" or row["City"] == "San diego":
        return row
    ...
    if location:
        ...
        print(
            f"Coordinates for Row {index}: {row['City']}, {row['State']} found: {location.latitude}, {location.longitude}"
        )
